...
// Rising three-note chime for Hero Mode activation
export function playHeroActivate() {
  const ac = ctx()
  const t = ac.currentTime
...
// Descending chime for Hero Mode deactivation
export function playHeroDeactivate() {
  const ac = ctx()
  const t = ac.currentTime
